...
import * as strings from 'helloKnockoutStrings';
import HelloKnockoutViewModel, { IHelloKnockoutBindingContext } from './HelloKnockoutViewModel';
import { IHelloKnockoutWebPartProps } from './IHelloKnockoutWebPartProps';
  
let _instance: number = 0;

export default class HelloKnockoutWebPart extends BaseClientSideWebPart<IHelloKnockoutWebPartProps> {
  private _id: number;
  ...
  protected onInit(): Promise<void> {
    ...
    const bindings: IHelloKnockoutBindingContext = {
  ...
  private _registerComponent(tagName: string): void {
    ko.components.register(
      tagName,
      {
        viewModel: HelloKnockoutViewModel,
        template: require('./HelloKnockout.template.html'),
        synchronous: false
      }
    );
  }
